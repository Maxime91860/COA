...
enum mpi_collective_code td3_q1_q2_q3_is_mpi_call( gimple * stmt )
{
	...
	{
		printf( "    -->   |||++|||      [MPI CALL] Found call to %s\n", mpi_collective_name[returned_code] ) ;
	}
...
void td2_q8_print_called_functions( basic_block bb )
{
	...
	for (gsi = gsi_start_bb (bb); !gsi_end_p (gsi); gsi_next (&gsi))
	{
		...
		c = td3_q1_q2_q3_is_mpi_call(stmt);
		bb->aux = (void *) LAST_AND_UNUSED_MPI_COLLECTIVE_CODE ;

		/******************************/
		/** TD3 - FIN QUESTION 1 > 4 **/
		/******************************/
	}
	/******************************/
	/**   TD2 - FIN QUESTION 8   **/
...
void clean_aux_field( function * fun )
{
	basic_block bb; 
