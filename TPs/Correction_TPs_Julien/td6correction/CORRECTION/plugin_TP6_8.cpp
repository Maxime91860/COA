		...
		bool gate (function *fun)
		{
			...
			if(ret)
			{
				...
				FOR_EACH_VEC_ELT_REVERSE (list_of_functions, ix, name)
				{
					...
					{
						list_of_functions.unordered_remove(ix);
					}
				}
			}
			/******************************/
...
void plugin_finalize(void *gcc_data, void *user_data) {
	if (list_of_functions.length()) {
		...
		printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
		printf("!!!! FUNCTION NAMES UNUSED !!!!\n");
		printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
		char *name;
		unsigned int ix;
	        FOR_EACH_VEC_ELT_REVERSE (list_of_functions, ix, name)
        	{
			printf ("argument %s not used (function is not defined)\n", name);
        	}	
	}
	list_of_functions.release();
}
