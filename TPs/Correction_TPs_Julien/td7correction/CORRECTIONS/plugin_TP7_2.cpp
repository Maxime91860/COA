...
void td5_bitmap_and_pdf_it(function * fun, bitmap_head ipdf_set[])
{
	/******************************/
	/****   TD5 - QUESTION 1   ****/
	/******************************/
...
void td7_insert_print_and_CCfunc(function * fun, bitmap_head ipdf_set[])
{
				...
				if ( bb->aux == (void *) i) 
				{
					...
					for (gsi = gsi_start_bb (bb); !gsi_end_p (gsi); gsi_next (&gsi))
					{
						...
						if ( c != LAST_AND_UNUSED_MPI_COLLECTIVE_CODE )
						{
							...
							for(int l=0; l<size_message; l++) message[l] = '\0';
					
							strncat(&message[0], " %s at line ", 12);
											
							char tmp_message[10];
							for(int l=0; l<10; l++) tmp_message[l] = '\0';

							sprintf(&tmp_message[0], "%d\n", gimple_lineno(stmt));
	
							strncat(&message[12], &tmp_message[0], 10);


							tree MPI_name_tree = fix_string_type( build_string (strlen(mpi_collective_name[i] ) + 1, mpi_collective_name[i]) );
							tree MPI_name_ptr = build_pointer_type (TREE_TYPE (TREE_TYPE (MPI_name_tree)));
							tree MPI_name_arg = build1 (ADDR_EXPR, MPI_name_ptr, MPI_name_tree);




							tree message_tree = fix_string_type( build_string (size_message+20, &message[0]));	
							tree message_ptr = build_pointer_type(TREE_TYPE (TREE_TYPE (message_tree)));
							tree message_arg = build1 (ADDR_EXPR, message_ptr, message_tree);



							gimple * cc_call = gimple_build_call (builtin_decl_explicit(BUILT_IN_PRINTF), 2, message_arg, MPI_name_arg);
...
class my_pass : public gimple_opt_pass
{
	public:
		...
		unsigned int execute (function *fun)
		{
			...
			td5_bitmap_and_pdf_it(fun, ipdf_set);
			...
			td_isol_print(/*TD*/7);
			td7_insert_print_and_CCfunc(fun, ipdf_set);
